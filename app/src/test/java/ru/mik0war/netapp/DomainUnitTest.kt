...
class DomainUnitTest {
    ...
    @Test
    fun `getList should return list with one error model when repository throws exception with message`() = runTest {
        // given
        ...
        assertEquals(ItemModel.Error(errorMessage), errorItem)
        coVerify(exactly = 1) { repository.getList() }
    }

    @Test
    fun `getList should return list with one error model with default message when exception message is null`() = runTest {
        // given
        ...
        assertEquals(1, result.size)
        val errorItem = result[0]
        assertEquals(ItemModel.Error("Unexpected error"), errorItem)
        coVerify(exactly = 1) { repository.getList() }
    }
    ...
    @Test
    fun `getItem should return error model when repository throws exception with message`() = runTest {
        // given
        val id = 99
        ...
        // then
        assertEquals(ItemModel.Error(errorMessage), result)
        coVerify(exactly = 1) { repository.getItem(id) }
    }

    @Test
    fun `getItem should return error model with default message when exception message is null`() = runTest {
        // given
        val id = 1
        ...
        // then
        assertEquals(ItemModel.Error("Unexpected error"), result)
        coVerify(exactly = 1) { repository.getItem(id) }
    }
